...
public class BattlePanel extends JPanel {
    ...
    public BattlePanel(BattleManager manager) {
        ...
        this.setFocusable(true); // Allows the panel to capture key presses
        ...
        // Add this inside the BattlePanel constructor
    ...
    private void checkMeleeRange() {
        // Pythagorean theorem to check distance between player and bot
        double distance = Math.sqrt(
                Math.pow(manager.player.x - manager.bot.x, 2) +
                        Math.pow(manager.player.y - manager.bot.y, 2)
        );

        // If closer than 100 pixels, the attack lands
        if (distance < 100) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Draw Characters at their current X and Y
        g2.drawImage(manager.player.getCurrentFrame(),
                manager.player.x, manager.player.y, 128, 128, null);

        g2.drawImage(manager.bot.getCurrentFrame(),
                manager.bot.x, manager.bot.y, 128, 128, null);

        // Draw Health Bars above their heads
